  ...
  const handleConfirmAction = async () => {
    ...
    try {
      ...
      if (confirmType === "approve") {
        res = await approveDepositRequest(selectedRequest.id);
      } else {
        res = await rejectDepositRequest(selectedRequest.id);
      }

      if (res && res.status === 1) {
        toast.success(res.message || `Deposit request ${confirmType}d successfully`);
        setConfirmType(null);
        setSelectedRequest(null);
        fetchDepositRequests(currentPage);
      } else {
        toast.error(res?.message || `Failed to ${confirmType} deposit request`);
      }
    } catch (err) {
    ...
    <>
      ...
      {confirmType && selectedRequest && (
        <LogoutModal
          message={
            confirmType === "approve"
              ? `Are you sure you want to approve the deposit request of ${selectedRequest.amount}?`
              : `Are you sure you want to reject the deposit request of ${selectedRequest.amount}?`
          }
